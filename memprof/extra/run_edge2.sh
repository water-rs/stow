#!/bin/bash
# usage: run_edge2.sh <build_dir> <tag> <route> <body_json>
set -u
export PATH="$HOME/node/bin:$HOME/.cargo/bin:$PATH"
B=$1; T=$2; ROUTE=$3; BODY=$4; cd ~/memprof; mkdir -p runs; R=runs/$T
...
node v8sample.mjs $R-v8.jsonl 900000 > $R-v8-summary.json 2>&1 &
SP=$!
