#!/bin/bash
# usage: native.sh <resolve-memprof binary> <suffix>
# Native runs of the same chain over recorded fixtures (record once with
# `--record <fixture dir>` in place of `--replay`). Writes each target's
# serialized (units, roots) to out/<x>-<suffix>.tsv; with a dhat build the
# profile lands in ./dhat-heap.json of the working directory.
BIN=$1; S=$2; D=${MEMPROF_DIR:-$HOME/memprof}; cd "$D"; mkdir -p out
...
for x in zed bat ra; do echo "== $x"; grep -h "User time\|Maximum resident" out/$x-$S.log
  grep -h '"label":"request_end"' out/$x-$S.log | python3 -c 'import sys,json;[print("native peak MiB %.1f"%(json.loads(l.split("MEMPROF ",1)[1])["peak"]/2**20)) for l in sys.stdin]'; done
