...
| `STOW_SERVABLE_UNITS_JSON` | wired by parent `stow check`/`build`/`test` | JSON-encoded `[[crate_name, version], ...]` serve map the supervising run computed once: the units its local cache and verified index can serve, plus `[crate_name, "*"]` entries the semantic fallback, prefetch candidates and name-scoped local hits may cover. A facade holding it answers the serve question without a supervisor round trip — a unit outside the map compiles directly. |
| `STOW_SERVE_MAP_FILE` | wired by parent `stow check`/`build`/`test` | Path of the `stow-serve-map.<build>.json` file the facade reads when `STOW_SERVABLE_UNITS_JSON` is absent — set when the verified index had not all reached disk before cargo started. The file carries the same serve-map JSON, is rewritten in place by the driver's background index fetch, and a missing or unreadable file means an empty map: nothing is known to serve yet, so the unit compiles directly. |
| `STOW_CC_PENDING_JOURNAL` | wired by parent `stow check`/`build`/`test` | Path of the `stow-cc-pending.<build>.jsonl` journal this build's `stow cc` facades append to. The supervising run sets it only when the local C-object store is empty — the once-per-build decision that no lookup can hit — so the facade execs the compiler and journals the compile for the drain to key, store and count, instead of paying the lookup pipeline. Unset (warm store, standalone cargo) means the ordinary `stow cc` lookup path. |
