...
impl<'gctx> PackageSet<'gctx> {
    ...
    pub async fn get_one(&self, id: PackageId) -> CargoResult<&Package> {
        if let Some(pkg) = self.packages.get(&id).and_then(|slot| slot.get()) {
            return Ok(pkg);
        }
        Ok(self.get_many(Some(id)).await?.remove(0))
    }
    ...
    pub async fn get_many(
        ...
        return Downloads::download(self, ids).await;
    }

    /// Downloads any packages accessible from the give root ids.
    #[tracing::instrument(skip_all)]
    pub async fn download_accessible(
        &self,
        resolve: &Resolve,
        root_ids: &[PackageId],
        ...
        let to_download = to_download
            ...
            .map(|(p, _)| p)
            .collect::<BTreeSet<_>>();
        self.get_many(to_download.into_iter()).await?;
        Ok(())
    }
    ...
    pub(crate) async fn warn_no_lib_packages_and_artifact_libs_overlapping_deps(
        &self,
        ws: &Workspace<'gctx>,
        resolve: &Resolve,
        root_ids: &[PackageId],
        ...
        let mut no_lib_pkgs: BTreeMap<PackageId, Vec<(&Package, &HashSet<Dependency>)>> =
            BTreeMap::new();
        for &root_id in root_ids {
            ...
            let mut dep_pkgs_and_deps = Vec::new();
            for (dep_package_id, deps) in dep_pkgs_to_deps {
                if !deps.iter().any(|dep| dep.maybe_lib()) {
                    continue;
                }
                if let Ok(dep_pkg) = self.get_one(dep_package_id).await {
                    if !dep_pkg.targets().iter().any(|t| t.is_lib()) {
                        dep_pkgs_and_deps.push((dep_pkg, deps));
                    }
                }
            }
            no_lib_pkgs.insert(root_id, dep_pkgs_and_deps);
        }
