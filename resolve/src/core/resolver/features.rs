...
impl<'a, 'gctx> FeatureResolver<'a, 'gctx> {
    ...
    async fn do_resolve(
        ...
        for (member, cli_features) in &member_features {
            let fvs = self
                .fvs_from_requested(member.package_id(), cli_features)
                .await;
            let fk = if self.track_for_host && self.has_any_proc_macro(member.package_id()).await {
            ...
            self.activate_pkg(member.package_id(), fk, &fvs).await?;
        }
        Ok(())
    }
    ...
    async fn activate_pkg(
        ...
        for fv in fvs {
            self.activate_fv(pkg_id, fk, fv).await?;
        }
        ...
        for &(dep_pkg_id, ref deps) in self.deps(pkg_id, fk).await?.iter() {
            for &(dep, dep_fk) in deps.iter() {
                ...
                Box::pin(self.activate_pkg(dep_pkg_id, dep_fk, &fvs)).await?;
            }
        }
        Ok(())
    }
    ...
    async fn activate_fv(
        ...
        match fv {
            FeatureValue::Feature(f) => {
                self.activate_rec(pkg_id, fk, *f).await?;
            }
            FeatureValue::Dep { dep_name } => {
                self.activate_dependency(pkg_id, fk, *dep_name).await?;
            }
    ...
    async fn activate_rec(
        ...
        for fv in fvs {
            Box::pin(self.activate_fv(pkg_id, fk, fv)).await?;
        }
        Ok(())
    }

    /// Activate a dependency (`dep:dep_name` syntax).
    async fn activate_dependency(
        &mut self,
        pkg_id: PackageId,
        fk: FeaturesFor,
        dep_name: InternedString,
            ...
            for &(dep, dep_fk) in deps.iter() {
                ...
                if let Some(to_enable) = &to_enable {
                    for dep_feature in to_enable {
                        ...
                        Box::pin(self.activate_fv(dep_pkg_id, dep_fk, &fv)).await?;
                    }
                }
                let fvs = self.fvs_from_dependency(dep_pkg_id, dep);
                Box::pin(self.activate_pkg(dep_pkg_id, dep_fk, &fvs)).await?;
                ...
                if dep.is_optional() {
                    ...
                    if !weak {
                        ...
                        if feature_map.contains_key(&dep_name) {
                            Box::pin(self.activate_rec(pkg_id, fk, dep_name)).await?;
                        }
                    }
                }
    ...
    async fn deps(
        ...
        async fn artifact_features_for(
        ...
        for (dep_id, deps) in self.resolve.deps(pkg_id) {
            let mut deps = deps
                ...
                .filter(|dep| {
                    if dep.platform().is_some()
                        ...
                        && !platform_activated(dep, fk, self.target_data, self.requested_targets)
                    {
                        return false;
                    }
            ...
            for dep in deps {
                ...
                let lib_fk = if fk != FeaturesFor::HostDep
                    && self.track_for_host
                    && (dep.is_build() || self.has_proc_macro_lib(dep_id).await)
                {
                    FeaturesFor::HostDep
                } else {
                    fk
                ...
                let dep_fks =
                    artifact_features_for(self, pkg_id, dep, lib_fk, unstable_json_spec).await?;
                dep_results.extend(dep_fks.into_iter().map(move |dep_fk| (dep, dep_fk)));
            }
    ...
    async fn has_any_proc_macro(&self, package_id: PackageId) -> bool {
        self.package_set
            .get_one(package_id)
            .await
    ...
    async fn has_proc_macro_lib(&self, package_id: PackageId) -> bool {
        self.package_set
            .get_one(package_id)
            .await
            .expect("packages downloaded")
