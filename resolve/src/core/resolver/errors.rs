...
pub(super) async fn activation_error(
    ...
    if let Some(version_candidates) = rejected_versions(registry, dep).await {
    ...
    } else if let Some(candidates) = alt_versions(registry, dep).await {
    ...
    } else if let Some(name_candidates) = alt_names(registry, dep).await {
    ...
    if has_too_new {
        let downgrade_to = alt_versions(registry, dep)
            .await
            .and_then(|r| r.ok())
            .and_then(|candidates| {
                candidates
...
async fn alt_versions(
    ...
    let candidates = match registry.query_vec(&wild_dep, QueryKind::Exact).await {
...
async fn rejected_versions(
    registry: &impl Registry,
    dep: &Dependency,
) -> Option<CargoResult<Vec<IndexSummary>>> {
    let mut version_candidates = match registry.query_vec(&dep, QueryKind::RejectedVersions).await {
        Ok(candidates) => candidates,
        Err(e) => return Some(Err(e)),
    };
...
async fn alt_names(
    ...
    let name_candidates = match registry
        .query_vec(&wild_dep, QueryKind::AlternativeNames)
        .await
    {
        Ok(candidates) => candidates,
        Err(e) => return Some(Err(e)),
