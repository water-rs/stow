...
impl SourceId {
    ...
    ///
    /// ```
    /// use stow_resolve::core::SourceId;
    ...
    pub fn local_path(self) -> Option<PathBuf> {
        ...
        crate::util::urls::url_to_path(&self.inner.url).ok()
    }
    ...
    pub fn load<'a>(self, gctx: &'a GlobalContext) -> CargoResult<Box<dyn Source + 'a>> {
        ...
        match self.inner.kind {
            ...
            SourceKind::Path => {
                ...
                if crate::util::toml::is_embedded(&path) && crate::util::fs::is_file(&path) {
    ...
    pub fn stable_hash<S: hash::Hasher>(self, workspace: &Path, into: &mut S) {
        if self.is_path() {
            if let Ok(p) = crate::util::urls::url_to_path(&self.inner.url)
                .unwrap()
...
fn url_display(url: &Url) -> String {
    if url.scheme() == "file" {
        if let Ok(path) = crate::util::urls::url_to_path(&url) {
