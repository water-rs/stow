...
pub async fn resolve_ws<'a>(
    ws: &Workspace<'a>,
    dry_run: bool,
) -> CargoResult<(PackageSet<'a>, Resolve)> {
    let mut registry = ws.package_registry()?;
    let resolve = resolve_with_registry(ws, &mut registry, dry_run).await?;
...
pub async fn resolve_ws_with_opts<'gctx>(
    ws: &Workspace<'gctx>,
    ...
    let (resolve, resolved_with_overrides) = if ws.ignore_lock() {
        ...
        .await?;
        ops::print_lockfile_changes(ws, None, &resolved_with_overrides, &mut registry).await?;
        ...
        // overrides, etc.
        add_overrides(&mut registry, ws).await?;
...
async fn resolve_with_registry<'gctx>(
    ws: &Workspace<'gctx>,
    ...
    if print {
        ops::print_lockfile_changes(ws, prev.as_ref(), &resolve, registry).await?;
    }
    Ok(resolve)
}
...
#[tracing::instrument(skip_all)]
pub async fn resolve_with_previous<'gctx>(
    registry: &mut PackageRegistry<'gctx>,
    ws: &Workspace<'gctx>,
    cli_features: &CliFeatures,
    has_dev_units: HasDevUnits,
    ...
    let avoid_patch_ids = if register_patches {
        register_patch_entries(registry, ws, previous, &mut version_prefs, keep_previous).await?
    } else {
        HashSet::new()
    };
    ...
    if register_patches && !resolved.unused_patches().is_empty() {
        emit_warnings_of_unused_patches(ws, &resolved, registry).await?;
    }
...
fn register_previous_locks(
    ...
    let path_pkg = |id: SourceId| {
        ...
        if let Ok(path) = crate::util::urls::url_to_path(&id.url()) {
...
async fn emit_warnings_of_unused_patches(
    ws: &Workspace<'_>,
    resolve: &Resolve,
...
async fn register_patch_entries(
    registry: &mut PackageRegistry<'_>,
    ws: &Workspace<'_>,
    previous: Option<&Resolve>,
    ...
    for (url, patches) in ws.root_patch()?.iter() {
        ...
        let Some(previous) = previous else {
            ...
            let unlock_ids = registry.patch(url, &patches).await?;
        ...
        for (orig_patch, unlock_id) in registry.patch(url, &registrations).await? {
