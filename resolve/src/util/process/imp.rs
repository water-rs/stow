...
/// Alias so vendored code can name the jobserver client type on both platforms.
pub type JobserverClient = Client;
...
impl ProcessBuilder {
    ...
    pub fn exec_with_streaming(
        ...
        let status = (|| {
            ...
            read2::read2(out, err, &mut |is_out, data, eof| {
                let pos = if is_out {
                    &mut stdout_pos
                } else {
                    &mut stderr_pos
...
pub(crate) fn debug_force_argfile(retry_enabled: bool) -> bool {
...
pub(crate) fn close_tempfile_and_log_error(file: NamedTempFile) {
