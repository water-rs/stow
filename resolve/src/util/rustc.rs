...
impl Rustc {
    ...
    pub fn new(
        ...
        Self::from_verbose_version_parts(path, wrapper, workspace_wrapper, verbose_version, cache)
    }
    ...
    pub fn new_from_verbose_version(path: PathBuf, verbose_version: String) -> CargoResult<Rustc> {
        Self::from_verbose_version_parts(
            path,
            None,
            None,
            verbose_version,
    ...
    /// Runs the compiler at `path` to learn various pieces of information —
    /// kept for non-wasm callers; `new` is unreachable where the rustc
    /// identity is injected. See [`Rustc::new_from_verbose_version`].
        ...
        if let std::collections::hash_map::Entry::Vacant(e) = self.data.outputs.entry(key) {
            ...
            e.insert(Output {
                ...
                status: if output.status.success() {
                    ...
                    crate::util::process::exit_status_to_string(output.status)
                },
