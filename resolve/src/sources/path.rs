...
impl<'gctx> Source for PathSource<'gctx> {
    ...
    fn describe(&self) -> String {
        match crate::util::urls::url_to_path(&self.source_id.url()) {
...
fn _list_files(pkg: &Package, gctx: &GlobalContext) -> CargoResult<Vec<PathEntry>> {
    ...
    // On wasm `git_repo` is always `None` (no real git can exist in the VFS),
    // so this whole dispatch is host-only.
    #[cfg(not(target_family = "wasm"))]
    if no_include_option {
...
/// On `wasm32-unknown-unknown` the VFS cannot contain a real git repository,
/// so discovery always reports `None` — the same answer cargo gives for a
/// directory that is not inside a repo, which selects `list_files_walk` below.
#[cfg(target_family = "wasm")]
fn discover_gix_repo(_root: &Path) -> CargoResult<Option<()>> {
    Ok(None)
}
