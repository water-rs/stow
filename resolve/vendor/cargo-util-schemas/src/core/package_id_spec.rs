...
impl PackageIdSpec {
    ...
    pub fn parse(spec: &str) -> Result<PackageIdSpec> {
        if spec.contains("://") {
            ...
            if abs.exists() {
                // `url::Url::from_file_path` is cfg'd out on wasm32-unknown-unknown;
                // this suggestion is only error-message text, so it degrades to the
                // generic hint cargo already uses when the conversion fails.
                let maybe_url = maybe_file_url(&abs);
...
fn maybe_file_url(abs: &std::path::Path) -> String {
    Url::from_file_path(abs).map_or_else(|_| "a file:// URL".to_string(), |url| url.to_string())
}

#[cfg(target_family = "wasm")]
fn maybe_file_url(_abs: &std::path::Path) -> String {
    "a file:// URL".to_string()
}
