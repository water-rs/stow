...
pub fn count_cc_pending(journal: &Path) -> u64 {
    ...
    contents
        .lines()
        .filter(|line| {
            serde_json::from_str::<CcPendingEntry>(line)
                .is_ok_and(|entry| entry.success)
        })
        .count() as u64
}

/// The pending-cc journals whose builds exited, beside the miss
/// journals [`finished_journals`] lists.
    ...
    entries
        ...
        .filter_map(|entry| {
            ...
            let build = name
                ...
                .strip_suffix(CC_PENDING_SUFFIX)?;
            build_finished(build).then(|| entry.path())
        })
        .collect()
}
...
async fn drain_cc_pending(config: &StowConfig, journal: &Path) {
    ...
    for (line_number, line) in contents.lines().enumerate() {
        ...
        let Ok(entry) = serde_json::from_str::<CcPendingEntry>(line) else {
            tracing::warn!(journal = %journal.display(), line = line_number + 1, "skipping a malformed pending C compile line");
            dropped = true;
            continue;
        };
        ...
        if let Err(error) = replay_cc_pending(config, &compiler, &args, entry.success, &mut fingerprints).await {
            tracing::warn!(error = %error, journal = %journal.display(), line = line_number + 1, "failed to store a deferred C compile");
            dropped = true;
        }
    }
    if dropped {
        // Whatever could not be replayed stays on disk for a later drain
        // to retry, the same contract the miss journal keeps.
        let _ = std::fs::rename(&work, journal);
    } else {
